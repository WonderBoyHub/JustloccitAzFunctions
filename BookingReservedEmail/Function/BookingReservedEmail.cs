...
{
    public class BookingReservedEmail
    {
        private readonly ILogger<BookingReservedEmail> _logger;
        private readonly EmailClient _emailClient;
        private readonly string _adminEmails;
        private readonly string _senderAddress;

        public BookingReservedEmail(ILogger<BookingReservedEmail> logger, EmailClient emailClient)
        {
            _logger = logger;
        ...
        [Function(nameof(BookingReservedEmail))]
        public async Task Run([EventGridTrigger] CloudEvent cloudEvent)
        {
            ...
            try
            {
                if (cloudEvent.Type != "Justloccit.Booking.Reserved")
                {
            ...
            {
                _logger.LogError(ex, "Error processing booking reserved event");
            }
        }
        ...
        private async Task SendCustomerEmailAsync(BookingEventData bookingData)
        {
            try
            {
                var subject = $"Booking Reservation Confirmation - {bookingData.Date:d}";
        ...
        private async Task SendAdminEmailAsync(BookingEventData bookingData)
        {
            try
            {
                var subject = $"New Booking Reservation - {bookingData.Date:d}";
        ...
        private string CreateCustomerEmailHtml(BookingEventData bookingData)
        {
            ...
            sb.AppendLine(".booking-status { display: inline-block; padding: 5px 10px; border-radius: 3px; background-color: #ffc107; color: #000; }");
            sb.AppendLine(".footer { margin-top: 30px; padding-top: 10px; border-top: 1px solid #ddd; font-size: 12px; color: #777; }");
            sb.AppendLine("</style>");
            sb.AppendLine("</head>");
            sb.AppendLine("<body>");
            sb.AppendLine("<div class='container'>");
            sb.AppendLine($"<h1>Booking Reservation Confirmation</h1>");
            sb.AppendLine($"<p>Dear {bookingData.CustomerName},</p>");
            sb.AppendLine("<p>Thank you for reserving a booking with Justloccit. Your booking details are as follows:</p>");
            ...
            sb.AppendLine($"<p><strong>Status:</strong> <span class='booking-status'>Pending</span></p>");
            sb.AppendLine("</div>");
            sb.AppendLine("<p>Please note that this booking is currently reserved and will need to be confirmed. We will send you a confirmation email once your booking is processed.</p>");
            sb.AppendLine("<p>If you have any questions, please feel free to contact us.</p>");
        ...
        private string CreateAdminEmailHtml(BookingEventData bookingData)
        {
            ...
            sb.AppendLine("<body>");
            sb.AppendLine("<div class='container'>");
            sb.AppendLine($"<h1>New Booking Reservation</h1>");
            sb.AppendLine("<p>A new booking reservation has been created with the following details:</p>");
            sb.AppendLine("<div class='booking-details'>");
            sb.AppendLine($"<p><strong>Booking ID:</strong> {bookingData.Id}</p>");
            sb.AppendLine($"<p><strong>Date:</strong> {bookingData.Date:dddd, MMMM d, yyyy}</p>");
            sb.AppendLine($"<p><strong>Time:</strong> {FormatTimeSpan(bookingData.StartTime)} - {FormatTimeSpan(bookingData.EndTime)}</p>");
            ...
            sb.AppendLine("</div>");
            sb.AppendLine("<p>This booking is currently in pending status and may require your attention.</p>");
            sb.AppendLine("<div class='footer'>");
