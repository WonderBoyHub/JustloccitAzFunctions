...
namespace Justloccit.Function
{
    ...
    {
        private readonly ILogger<BookingCancelledEmail> _logger;
        private readonly EmailClient _emailClient;
        private readonly string _adminEmails;
        private readonly string _senderAddress;

        public BookingCancelledEmail(ILogger<BookingCancelledEmail> logger, EmailClient emailClient)
        {
            _logger = logger;
        ...
        [Function(nameof(BookingCancelledEmail))]
        public async Task Run([EventGridTrigger] CloudEvent cloudEvent)
        {
            ...
            try
            {
                if (cloudEvent.Type != "Justloccit.Booking.Cancelled")
                {
            ...
            {
                _logger.LogError(ex, "Error processing booking cancelled event");
            }
        }
        ...
        private async Task SendCustomerEmailAsync(BookingEventData bookingData)
        {
            try
            {
                var subject = $"Booking Cancellation - {bookingData.Date:d}";
        ...
        private async Task SendAdminEmailAsync(BookingEventData bookingData)
        {
            try
            {
                var subject = $"Booking Cancelled - {bookingData.Date:d}";
        ...
        private string CreateCustomerEmailHtml(BookingEventData bookingData)
        {
            ...
            sb.AppendLine(".booking-status { display: inline-block; padding: 5px 10px; border-radius: 3px; background-color: #dc3545; color: #fff; }");
            sb.AppendLine(".footer { margin-top: 30px; padding-top: 10px; border-top: 1px solid #ddd; font-size: 12px; color: #777; }");
            sb.AppendLine("</style>");
            sb.AppendLine("</head>");
            sb.AppendLine("<body>");
            sb.AppendLine("<div class='container'>");
            sb.AppendLine($"<h1>Booking Cancellation</h1>");
            sb.AppendLine($"<p>Dear {bookingData.CustomerName},</p>");
            sb.AppendLine("<p>Your booking with Justloccit has been cancelled. The details of the cancelled booking are as follows:</p>");
            ...
            sb.AppendLine($"<p><strong>Status:</strong> <span class='booking-status'>Cancelled</span></p>");
            sb.AppendLine("</div>");
            sb.AppendLine("<p>If you would like to make a new booking, please visit our website or contact us directly.</p>");
            sb.AppendLine("<p>If you believe this cancellation is in error, please contact us as soon as possible.</p>");
        ...
        private string CreateAdminEmailHtml(BookingEventData bookingData)
        {
            ...
            sb.AppendLine("<body>");
            sb.AppendLine("<div class='container'>");
            sb.AppendLine($"<h1>Booking Cancelled</h1>");
            sb.AppendLine("<p>A booking has been cancelled with the following details:</p>");
            sb.AppendLine("<div class='booking-details'>");
            sb.AppendLine($"<p><strong>Booking ID:</strong> {bookingData.Id}</p>");
            sb.AppendLine($"<p><strong>Date:</strong> {bookingData.Date:dddd, MMMM d, yyyy}</p>");
            sb.AppendLine($"<p><strong>Time:</strong> {FormatTimeSpan(bookingData.StartTime)} - {FormatTimeSpan(bookingData.EndTime)}</p>");
