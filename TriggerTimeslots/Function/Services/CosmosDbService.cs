...
namespace Justloccit.Services
{
    public interface ICosmosDbService
    {
        ...
        Task<ReservationDocument?> GetReservationAsync(string id, string partitionKey);
    }
    ...
    public class CosmosDbService : ICosmosDbService
    {
        ...
        public async Task<ReservationDocument?> GetReservationAsync(string id, string partitionKey)
        {
            try
            {
                var response = await _reservationsContainer.ReadItemAsync<ReservationDocument>(id, new PartitionKey(partitionKey));
                return response.Resource;
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
